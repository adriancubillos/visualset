...
export default function ImageViewer({
    ...
    return (
      <div
        className={`flex items-center justify-center bg-gray-100 rounded-lg ${
          size === 'small' ? 'w-16 h-16' : size === 'medium' ? 'w-32 h-32' : 'w-64 h-64'
        } ${className}`}>
        <svg
  ...
  return (
    <>
      {/* Thumbnail */}
      <div
        className={`relative ${sizeClasses[size]} rounded-lg overflow-hidden border-2 border-gray-200 ${
          showModal ? 'cursor-pointer hover:border-blue-500 transition-colors' : ''
        } ${className}`}
        onClick={(e) => {
          e.stopPropagation();
